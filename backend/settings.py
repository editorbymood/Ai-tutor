...
MIDDLEWARE = [
    ...
    # "whitenoise.middleware.WhiteNoiseMiddleware",  # Commented out - whitenoise not installed
...
# Use MongoDB via djongo in normal runs. For tests (PYTEST_CURRENT_TEST set) or when
# DJONGO_DISABLED is set, fall back to SQLite so the suite can run without Mongo.
if os.getenv("PYTEST_CURRENT_TEST") or config("DJONGO_DISABLED", default=False, cast=bool):
    DATABASES = {
        "default": {
...
# STATICFILES_STORAGE = "whitenoise.storage.CompressedManifestStaticFilesStorage"  # Commented out - whitenoise not installed
...
SIMPLE_JWT = {
    "ACCESS_TOKEN_LIFETIME": timedelta(
        minutes=int(config("JWT_ACCESS_TOKEN_LIFETIME", default=60, cast=int) or 60)
    ),
    "REFRESH_TOKEN_LIFETIME": timedelta(
        minutes=int(config("JWT_REFRESH_TOKEN_LIFETIME", default=1440, cast=int) or 1440)
    ),
...
DEFAULT_AI_PROVIDER = config("DEFAULT_AI_PROVIDER", default="hybrid")  # gemini, openai, or hybrid
